...
def _end_skill(agent: str) -> str:
    return "\n".join((
        ...
        "4. Split the session into one concise structured Session Handoff per distinct completed work unit: goal, problem, prior approach, why it failed, alternatives, chosen fix, rationale, validation, risk, unresolved work, and optional structured rule fields.",
        "5. Call `forge_complete_session` once with the session ID and `handoffs: [...]`. It saves every handoff before making the lease ready to end. Each handoff requires: agent, worktree_path, branch, outcome_key, scope, category, goal, problem, prior_approach, why_prior_approach_failed, alternatives, chosen_fix, rationale, validation, risk, unresolved, proposed_rule, and evidence_span_ids. Use `unresolved` (not `unresolved_work`) and set `proposed_rule` to `none` when no rule is proposed.",
        "   Use this item shape inside `handoffs`: `{agent: 'antigravity', worktree_path: '.', branch: '<current branch>', outcome_key: '<unique work-unit key>', scope: ['repository'], category: '<work category>', goal: '<goal>', problem: '<problem>', prior_approach: '<prior approach>', why_prior_approach_failed: '<why>', alternatives: [], chosen_fix: '<fix>', rationale: '<why this fix>', validation: '<result>', risk: '<risk>', unresolved: '<unresolved work or none>', proposed_rule: 'none', evidence_span_ids: ['<persisted evidence span id>']}`. Add learning fields only when proposing a rule.",
