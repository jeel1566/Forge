...
type Today = { intention: { statement?: string; reason?: string } };
...
function App() {
  ...
  const loadWorkspace = () => {
    void get<Today>(`/v1/workspaces/${workspace}/today`, { intention: {} }).then(setToday);
  ...
  const saveIntention = async (event: React.FormEvent) => { event.preventDefault(); if (!intention.trim()) return; await fetch(`${api}/v1/workspaces/${workspace}/intention`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ statement: intention }) }); setIntention(""); loadWorkspace(); };
  ...
  return <main>
    ...
    <section><h2>Active intention</h2>{today.intention.statement ? <p className="intention">{today.intention.statement}</p> : <p className="muted">{today.intention.reason || "No active intention."}</p>}<form onSubmit={saveIntention}><input aria-label="Active intention" value={intention} onChange={event => setIntention(event.target.value)} placeholder="e.g. Keep pull requests easy to review"/><button>Set intention</button></form></section>
