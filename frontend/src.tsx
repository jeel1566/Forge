...
type SessionHandoff = { id: string; agent: string; branch: string; scope: string[]; category: string; goal: string; problem: string; prior_approach: string; why_prior_approach_failed: string; alternatives: string[]; chosen_fix: string; rationale: string; validation: string; risk: string; unresolved: string; proposed_rule: string; created_at: string; citations: Citation[] };
...
function normalizeHandoff(handoff: SessionHandoff): SessionHandoff { return { ...handoff, scope: safeList<string>(handoff.scope), alternatives: safeList<string>(handoff.alternatives), citations: safeList<Citation>(handoff.citations) }; }
  ...
  return <div className="app-shell">
    ...
    <main className="main-content">
      ...
      <div className="page-wrap">
        ...
        {view === "handoffs" && <section className="workspace-page"><div className="page-title"><div><p className="eyebrow">TRANSCRIPT-FREE CONTINUITY</p><h1>The work stays legible.</h1><p>Clean handovers, written by agents and grounded in persisted citations.</p></div><span className="big-count">{handoffs.length}<small>handoffs</small></span></div><div className="handoff-grid">{handoffs.length ? handoffs.map(handoff => <article className="panel handoff-card" key={handoff.id}><div className="handoff-meta"><span className="agent-pill">{handoff.agent}</span><small>{timeLabel(handoff.created_at)}</small></div><h2>{handoff.goal}</h2><div className="handoff-flow"><p><span>Problem</span>{handoff.problem}</p><p><span>What changed</span>{handoff.chosen_fix}</p><p><span>Why it works</span>{handoff.rationale}</p></div><details className="handoff-detail"><summary>View full handoff</summary><div className="handoff-flow"><p><span>Prior approach</span>{handoff.prior_approach}</p><p><span>Why it failed</span>{handoff.why_prior_approach_failed}</p>{handoff.alternatives.length > 0 && <p><span>Alternatives</span>{handoff.alternatives.join(" · ")}</p>}<p><span>Validation</span>{handoff.validation}</p><p><span>Risk</span>{handoff.risk}</p><p><span>Unresolved</span>{handoff.unresolved}</p><p><span>Citations</span>{handoff.citations.map(citation => citation.quote).join(" · ")}</p></div></details><footer><span>{handoff.category}</span><span>{handoff.scope.join(" · ")}</span><span>{handoff.citations.length} citations</span></footer></article>) : <Empty title="No handoffs recorded" detail="A future /forge_end records the compact, cited result of a session." />}</div></section>}
