...
function App() {
  ...
  const load = () => fetch(`${api}/v1/workspaces/default/today`).then(response => { if (!response.ok) throw new Error("Forge is offline or could not load Today."); return response.json(); }).then(setToday).catch(loadError => setError(loadError.message));
  ...
  const loadGitHubEvidence = () => fetch(`${api}/v1/workspaces/default/evidence?kind=github_pull_request`).then(response => response.json()).then(setGithubEvidence);
  const loadRepository = () => fetch(`${api}/v1/workspaces/default/repository`).then(response => response.ok ? response.json() : null).then(setRepository);
  const loadHistory = () => fetch(`${api}/v1/workspaces/default/history`).then(response => response.json()).then(setHistory);
  useEffect(() => { void load(); void loadGitHub(); void loadEvidence(); void loadGitHubEvidence(); void loadRepository(); void loadHistory(); }, []);
  ...
  const pollGitHub = async () => { await fetch(`${api}/v1/workspaces/default/github/poll`, { method: "POST" }); loadGitHub(); loadGitHubEvidence(); };
  ...
  const refreshEvidence = async () => { if (!repository) return; await fetch(`${api}/v1/workspaces/default/git/imports`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ path: repository.path }) }); loadEvidence(); loadRepository(); };
  ...
  return <main>
    ...
    <section><h2>Repository</h2>{repository ? <><p><code>{repository.path}</code></p><p className="muted">{repository.branch} · {repository.remote_url || "no remote"}</p><button onClick={refreshEvidence}>Refresh Git evidence</button></> : <p className="muted">No repository registered. Restart Forge with <code>forge start --path path-to-repository</code>.</p>}</section>
    ...
    <section><h2>GitHub evidence</h2>{githubEvidence.length ? <ul>{githubEvidence.map(item => <li key={item.id}><button className="link" onClick={() => showEvidence(item.id)}>{item.title}</button></li>)}</ul> : <p className="muted">No pull requests imported yet.</p>}</section>
